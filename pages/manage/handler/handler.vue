...
<template>
	<view style="height: 100vh;">
		...
		<scroll-view class="scrollviewpadding">
			...
			<view v-for="(item, index) in HandlerList" :key="item.id || index">
				<view class="good-item" style="min-height: 80rpx;">
					<u-row gutter="10">
						<u-col span="12">
							<u-collapse :head-style="headStyle">
								<u-collapse-item :title="`姓名：${item.name}`">
...
<script setup lang="ts">
	...
	import { getHandler } from '../../../api/api'
	import { $u, useTheme } from 'uview-pro'
	const { currentTheme, themes, darkMode } = useTheme();
	const title = ref<string>('经手人管理')
	...
	//加载收支项目列表
	const HandlerList = ref<Array>([]);
	const searchname = ref<string>('');
	const loadGetHandlerList = async () => {
		let params = {
		...
		const res = await getHandler(params)
		if (res && res.code === 200) {
			listTotal.value = res.data.total
			HandlerList.value = res.data.rows
