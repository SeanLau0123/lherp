...
<template>
	<view style="height: 100vh;">
		...
		<scroll-view class="scrollviewpadding">
			...
			<u-swipe-action :show="RetailOut.show" :index="index" v-for="(RetailOut, index) in RetailOutList"
				:key="RetailOut.id" @click="click" @open="open" :options="getSwipeOptions(RetailOut)"
				:btn-width="btnWidth">
				<view class="good-item">
					<u-row gutter="10">
						<u-col span="12">
							<u-collapse :head-style="headStyle">
								<u-collapse-item :title="`会员：${RetailOut.organName}`">
									<u-col span="12">
										<view class="goods-row">
											<text class="label">单据编号：</text>
											<u-text type="primary" decoration="underline"
												:text="(RetailOut.number || '')"
												@click="lookNumberDetail(RetailOut.number)"></u-text>
										</view>
									</u-col>
									<u-col span="12">
										<view class="goods-row">
											<text class="label">商品信息：</text>
											<u-text :text="RetailOut.materialsList"></u-text>
										</view>
									</u-col>
									<u-col span="12">
										<view class="goods-row">
											<text class="label">单据日期：</text>
											<u-text>{{ $u.timeFormat(RetailOut.billTime, 'yyyy-mm-dd hh:MM:ss')
												}}</u-text>
										</view>
									</u-col>
									<u-col span="12">
										<view class="goods-row">
											<text class="label">收款类型：</text>
											<u-text :text="RetailOut.payType"></u-text>
										</view>
									</u-col>
						...
						<u-col span="12">
							<view class="goods-row">
								...
								<u-text :text="RetailOut.userName"></u-text>
								<text class="label">数量：</text>
								<u-text :text="RetailOut.materialCount"></u-text>
							</view>
						</u-col>
						<u-col span="12">
							<view class="goods-row">
								<text class="label">收款金额：</text>
								<u-text mode="price" :text="RetailOut.changeAmount + RetailOut.backAmount"></u-text>
								<text class="label">找零：</text>
								<u-text mode="price" :text="RetailOut.backAmount"></u-text>
							</view>
						</u-col>
						<u-col span="12">
							<view class="goods-row">
								<text class="label">收款账户：</text>
								<u-text :text="RetailOut.accountName"></u-text>
							</view>
						</u-col>
						<u-col span="9">
							<view class="goods-row">
								<text class="label">金额合计：</text>
								<u-text mode="price" :text="RetailOut.totalPrice"></u-text>
							</view>
						</u-col>
						<u-col span="3">
							<view class="goods-row status-right-align">
								<text v-if="RetailOut.status == '0'"
									:style="{ color: $u.color.warning, fontSize: '32rpx', fontWeight: 'bold' }">未审核</text>
								<text v-if="RetailOut.status == '1'"
...
<script setup lang="ts">
	...
	const getSwipeOptions = (RetailOut : ListItem) : OptionButton[] => {
		return [
			{
				text: RetailOut.status === '1' ? "反审核" : "审核",
				style: {
	...
	// 删除确认
	const confirm = async () => {
		RetailOutList.value.splice(selectedIndex.value, 1)
		try {
	...
	const open = (index : number) => {
		RetailOutList.value[index].show = true
		RetailOutList.value.map((val, idx) => {
			if (index != idx) RetailOutList.value[idx].show = false
		})
	}
	// 定义点击事件回调函数
	const click = (index : number, index1 : number) => {
		const curItem = RetailOutList.value[index]; // 缓存当前项，避免多次取值
		...
		if (index1 === 2) {
			...
			goRetailOutDetail(curItem); // 调用已有编辑方法，传递当前单据数据
		} else {
	...
	const RetailOutList = ref<Array>([]);
	const billno = ref<string>('');
	const loadRetailOutList = async () => {
		let params = {
	...
	//新增
	function onBtnClick() {
		uni.$u.route('pages/openorder/RetailOut/AddRetailOut');
	}
	//编辑
	function goRetailOutDetail(item) {
		uni.$u.route('/pages/openorder/RetailOut/AddRetailOut?item=' + encodeURIComponent(JSON.stringify(item)) + '&action=edit')
	}
