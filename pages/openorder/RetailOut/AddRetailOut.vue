	...
	<view style="min-height: 100vh;">
		...
		<view class="popup-title">
			<u-form :model="myform" label-width="165rpx" :rules="rules" ref="uFormRef" label-align="right"
				...
				<u-form-item label-align="right" label="收款类型：">
					<u-input v-model="myform.payType" :type="type" placeholder="请选择收款类型"
						@click="openPayType" /></u-form-item>
				<u-select v-model="payTypeShow" :list="payTypeList" @confirm="payTypeConfirm"></u-select>
				...
				<u-row gutter="10">
					...
					<u-col span="12">
						<u-form-item label-align="right" label="收款金额：" prop="totalAmount">
							<u-input v-model="myform.totalAmount" placeholder="请输入收款金额" type="digit" @input="InputTotalAmount" /></u-form-item>
					</u-col>
					<u-col span="12">
					...
					<u-col span="12">
						<u-form-item label-align="right" label="*收款账户：">
							<u-input v-model="accountName" placeholder="请选择收款账户" @click="accountShow = true"
...
<script setup lang="ts">
	...
	const getOrderNo = async () => {
		...
		if (res && res.code === 200) {
			billno.value = "LSCK" + res.data.defaultNumber;
		}
	}
	...
	//收款类型打开事件
	const openPayType = async () => {
		  if (!myform.member) {
		    showToast('请先选择会员', 'warning');
		    return;
		  }
		await loadMemberPayType();
		payTypeShow.value = true;
		}
	
	// 选择会员后，加载对应收款类型
	const loadMemberPayType = async () => {
		try {
			let params = {
				search: JSON.stringify({
					supplier: memberName.value,
					type: "会员"
				})
			}
			const res = await getPartnerlList(params);
			if (res && res.code === 200) {
				payTypeList.value = []; // 清空原有收款类型
				const info = res.data.rows[0];
				// 根据预付款状态动态生成收款类型
				if (info.advanceIn) {
					payTypeList.value.push({
						value: "预付款", label: `预付款（${info.advanceIn}）`
					});
					payTypeList.value.push({ value: "现付", label: "现付" });
					
					payTypeId.value = "预付款";
					payTypeName.value = `预付款（${info.advanceIn}）`;
				} else {
					payTypeList.value.push({
						value: "现付", label: "现付"
					});
					payTypeId.value = "现付";
					payTypeName.value = "现付";
				}
				myform.payType = payTypeName.value; // 同步到表单
			}
		} catch (error) {
			showToast('收款类型加载失败', 'warning');
		}
	};
	//加载收款类型
	const payTypeShow = ref<boolean>(false)
	const payTypeName = ref<string>('');
	const payTypeId = ref<string>('');
	const payTypeList = ref<ListItem[]>([]);
	const loadPayTypeList = async () => {
		payTypeList.value.push({
			value: '现付',
			label: '现付'
		});
		payTypeName.value = payTypeList.value[0].label;
		payTypeId.value = payTypeList.value[0].value;
	}
	// 收款类型确认回调函数
	const payTypeConfirm = (e : any[]) => {
		payTypeName.value = e[0].label;
		payTypeId.value = e[0].value;
		myform.payType = payTypeName.value;
	}
	...
	const getSaleOutDetailData = async (billNumber : string) => {
		...
		if (res && res.code === 200) {
			...
			myform.payType = detail.payType || '';
			payTypeName.value = detail.payType;
			payTypeId.value = detail.payType;
	...
	//新增取消
	function Cancel() {
		uni.$u.route('pages/openorder/RetailOut/RetailOut');
	}
