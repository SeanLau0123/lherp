									...
									<u-col span="12">
										<view class="goods-row">
											...
											<u-text type="primary" decoration="underline"
												:text="(PurchaseIn.number || '') + (PurchaseIn.linkNumber ? '[订]' : '') + (PurchaseIn.hasBackFlag ? '[退]' : '')"
...
<script setup lang="ts">
	...
	const click = (index : number, index1 : number) => {
		...
		if (index1 === 2) {
			...
			goPurchaseInDetail(curItem); // 调用已有编辑方法，传递当前单据数据
		} else {
	...
	const loadPurchaseInList = async () => {
		let params = {
	...
	//新增
	function onBtnClick() {
		uni.$u.route('pages/openorder/InboundOrder/AddPurchaseIn');
	}
	//编辑
	function goPurchaseInDetail(item) {
		uni.$u.route('/pages/openorder/InboundOrder/AddPurchaseIn?item=' + encodeURIComponent(JSON.stringify(item)) + '&action=edit')
	}
	//分页切换
