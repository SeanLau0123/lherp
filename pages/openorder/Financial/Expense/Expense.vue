...
<template>
	<view style="height: 100vh;">
		...
		<scroll-view class="scrollviewpadding">
			...
			<u-swipe-action :show="income.show" v-for="(income, index) in incomeList"
				:key="income.id || index" :index="index" @click="click" @open="open"
				:options="getSwipeOptions(income)" :btn-width="btnWidth">
				<view class="good-item">
					<u-row gutter="10">
						<u-col span="12">
							<u-collapse :head-style="headStyle">
								<u-collapse-item :title="`往来单位：${income.organName}`">
									<u-col span="12">
										<view class="goods-row">
											<text class="label">单据编号：</text>
											<u-text type="primary" decoration="underline"
												:text="income.billNo" @click="lookNumberDetail(income.billNo)"></u-text>
										</view>
									</u-col>
									<u-col span="12">
										<view class="goods-row">
											<text class="label">单据日期：</text>
											<u-text>{{ $u.timeFormat(income.billTime, 'yyyy-mm-dd hh:MM:ss') }}</u-text>
										</view>
									</u-col>
						...
						<u-col span="12">
							<view class="goods-row">
								...
								<u-text :text="income.userName"></u-text>
								<text class="label">财务人员：</text>
								<u-text :text="income.handsPersonName"></u-text>
							</view>
						</u-col>
						<u-col span="12">
							<view class="goods-row">
								<text class="label">支出账户：</text>
								<u-text :text="income.accountName"></u-text>
							</view>
						</u-col>
						<u-col span="12">
							<view class="goods-row">
								<text class="label">支出金额：</text>
								<u-text mode="price" :text="income.changeAmount"></u-text>
							</view>
						</u-col>
<!-- 						<u-col span="12">
							<view class="goods-row">
								<text class="label">合计金额：</text>
								<u-text mode="price" :text="income.totalPrice"></u-text>
							</view>
						</u-col> -->
						<u-col span="9">
							<view class="goods-row">
								<text class="label">备注：</text>
								<u-text :text="income.remark"></u-text>
							</view>
						</u-col>
						<u-col span="3">
							<view class="goods-row status-right-align">
								<text v-if="income.status == '0'"
									:style="{color: $u.color.warning,fontSize: '32rpx',fontWeight: 'bold'}">未审核</text>
								<text v-if="income.status == '1'"
...
<script setup lang="ts">
	...
	const getSwipeOptions = (income : ListItem) : OptionButton[] => {
		return [
			{
				text: income.status === '1' ? "反审核" : "审核",
				style: {
	...
	// 删除确认
	const confirm = async () => {
		incomeList.value.splice(selectedIndex.value, 1)
		try {
	...
	const open = (index : number) => {
		incomeList.value[index].show = true
		incomeList.value.map((val, idx) => {
			if (index != idx) incomeList.value[idx].show = false
		})
	}
	// 定义点击事件回调函数
	const click = (index : number, index1 : number) => {
		const curItem = incomeList.value[index]; // 缓存当前项，避免多次取值
		...
		if (index1 === 2) {
			...
			goExpenseDetail(curItem); // 调用已有编辑方法，传递当前单据数据
		} else {
	...
	function lookNumberDetail(number : string) {
		uni.$u.route('pages/openorder/Financial/Expense/ExpenseDetail',
			{ number: number });
	}
	//新增
	function onBtnClick() {
		uni.$u.route('pages/openorder/Financial/Expense/AddExpense');
	}
	//编辑
	function goExpenseDetail(item) {
		uni.$u.route('/pages/openorder/Financial/Expense/AddExpense?item=' + encodeURIComponent(JSON.stringify(item)) + '&action=edit')
	}
	//分页切换
