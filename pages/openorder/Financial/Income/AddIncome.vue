			...
			<u-form :model="myform" label-width="160rpx" :rules="rules" ref="uFormRef" label-align="right"
				...
				<view class="rowinfo">
					...
					<scroll-view class="scrollviewpadding">
						<view v-for="(item, index) in incomeDetailList" :key="item.id || index">
							<u-row gutter="5">
								<u-col span="1">
				...
				<!-- 订单表尾 -->
				<u-form-item label-align="right" label="收入账户：" prop="account">
					<u-input v-model="myform.account" placeholder="请选择收入账户" @click="accountShow = true" :type="type"
						:disabled="true" />
			...
			<view style="padding: 10rpx 10rpx;">
				<view>
					<u-row gutter="10" justify="between">
						<view class="u-btn-picker u-btn-picker--tips" @click="incomeDetailClose">取消</view>
						<view class="u-btn-picker u-btn-picker--primary" @click="incomeDetailSubmit">确认</view>
					</u-row>
					<u-line></u-line>
				</view>
				<u-form label-width="170rpx">
					<u-form-item label-align="right" label="收入项目：" prop="income">
						<u-input v-model="incomeDetail.inOutItemName" placeholder="请选择收入项目" @click="incomeShow = true"
							:type="type" :disabled="true" />
						<u-select v-model="incomeShow" :list="incomeList" @confirm="IncomeConfirm"></u-select>
					</u-form-item>
					<u-row gutter="10">
						<u-col span="12">
							<u-form-item label-align="right" label="金额：">
								<u-input v-model="incomeDetail.eachAmount" type="digit" /></u-form-item>
						</u-col>
						<u-col span="12">
							<u-form-item label-align="right" label="备注：">
								<u-input v-model="incomeDetail.remark" /></u-form-item>
						</u-col>
					</u-row>
				</u-form>
			</view>
...
<script setup lang="ts">
	...
	const getOrderNo = async () => {
		...
		if (res && res.code === 200) {
			billno.value = "SR" + res.data.defaultNumber;
		}
	}
	...
	const loadGetIncomeList = async () => {
		...
		if (res) {
			...
			incomeDetail.value.inOutItemName = incomeName.value;
			incomeDetail.value.inOutItemId = incomeId.value;
		}
		else {
			uni.showToast({ title: '收入项目列表加载失败', icon: 'none' });
		}
	}
	// 收入项目列表确认回调函数
	const IncomeConfirm = (e : any[]) => {
	...
	const handleSelectAll = (value : boolean) => {
		incomeDetailList.value.forEach(item => {
			item.checked = value;
		});
		incomeDetailList.value = [...incomeDetailList.value];
	}

	const checkboxChange = (e : boolean, name : string) => {
		const index = parseInt(name)
		if (index >= 0 && index < incomeDetailList.value.length) {
			incomeDetailList.value[index].checked = e
		}
		// 同步全选
		const allChecked = incomeDetailList.value.every(item => item.checked)
	...
	function accountDelete() {
		// 过滤掉选中的行
		incomeDetailList.value = incomeDetailList.value.filter(item => !item.checked);
		// 重置全选
		selectAll.value = false;
		const total = incomeDetailList.value.reduce((sum, item) => {
			return sum + Number(item.eachAmount || 0);
		}, 0);
		myform.totalAmount = total;
	}
	const incomeDetailList = ref<AccountItem[]>([]);

	function addAccountToList(newAccountData) {
		if (!newAccountData.inOutItemName) {
		...
		incomeDetailList.value.push(newItem);
		incomeDetailList.value = [...incomeDetailList.value];
	}
	// 核心修改3：新增当前编辑行的索引变量
	...
	const loadIncomeItemList = async (headerId : string) => {
		...
		if (res && res.code === 200) {
			...
			incomeDetailList.value = formattedRows;
		}
		else {
	...
	function incomeDetailSubmit() {
		// 1. 数据验证
		if (!incomeDetail.value.inOutItemName) {
			showToast('请选择收入项目', 'warning');
			return;
		}
		if (!incomeDetail.value.eachAmount) {
			showToast('请输入金额', 'warning');
			return;
		}
		const newAccountData = {
			id: Date.now().toString(),
			inOutItemId: incomeDetail.value.inOutItemId,
			inOutItemName: incomeDetail.value.inOutItemName,
			eachAmount: incomeDetail.value.eachAmount,
			remark: incomeDetail.value.remark || '',
			checked: false
		};
		// 3. 添加到列表
		incomeDetailList.value.push(newAccountData);
		incomeDetailList.value = [...incomeDetailList.value];
	...
	const SavePurchaseIn = async () => {
		...
		if (incomeDetailList.value.length === 0) {
			showToast('单据明细不能为空', 'warning');
			return;
		}
		const totalAmount = incomeDetailList.value.reduce((sum, item) => {
		...
		const rowsParams = incomeDetailList.value.map((item, index) => {
		...
		if (res.code === 200) {
			...
			uni.$u.route('pages/openorder/Financial/Income/Income');
		} else {
	...
	onMounted(() => {
		//监听全局事件，接收新商品数据
		uni.$on('addincomeDetail', (newAccountData) => {
	...
	onLoad((option) => {
		...
		if (action === 'edit' && itemStr) {
			...
			isEdit.value = false;
			title.value = '新增';
			const incomeDetailStr = option.incomeDetail || '';
			if (incomeDetailStr) {
				const incomeDetailObj = JSON.parse(decodeURIComponent(incomeDetailStr));
				addAccountToList(incomeDetailObj);
			}
			initDefaultDates();
			getOrderNo();
		}
