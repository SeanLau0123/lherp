...
<script setup lang="ts">
	...
	const click = (index : number, index1 : number) => {
		...
		if (index1 === 2) {
			...
			goIncomeDetail(curItem); // 调用已有编辑方法，传递当前单据数据
		} else {
	...
	function lookNumberDetail(number : string) {
		uni.$u.route('pages/openorder/Financial/Income/IncomeDetail',
			{ number: number });
	}
	//新增
	function onBtnClick() {
		uni.$u.route('pages/openorder/Financial/Income/AddIncome');
	}
	//编辑
	function goIncomeDetail(item) {
		uni.$u.route('/pages/openorder/Financial/Income/AddIncome?item=' + encodeURIComponent(JSON.stringify(item)) + '&action=edit')
	}
	//分页切换
