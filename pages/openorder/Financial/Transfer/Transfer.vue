...
<template>
	<view style="height: 100vh;">
		...
		<scroll-view class="scrollviewpadding">
			...
			<u-swipe-action :show="advanceIn.show" v-for="(advanceIn, index) in TransferList"
					...
					<u-row gutter="10">
						<u-col span="12">
							<u-collapse :head-style="headStyle">
								<u-collapse-item :title="`付款账户：${advanceIn.accountName}`">
									<u-col span="12">
...
<script setup lang="ts">
	...
	// 删除确认
	const confirm = async () => {
		TransferList.value.splice(selectedIndex.value, 1)
		try {
	...
	const open = (index : number) => {
		TransferList.value[index].show = true
		TransferList.value.map((val, idx) => {
			if (index != idx) TransferList.value[idx].show = false
		})
	}
	// 定义点击事件回调函数
	const click = (index : number, index1 : number) => {
		const curItem = TransferList.value[index]; // 缓存当前项，避免多次取值
	...
	const TransferList = ref<Array>([]);
	const billno = ref<string>('');
	const loadGetTransferList = async () => {
		let params = {
		...
		if (res && res.code === 200) {
			...
			TransferList.value = res.data.rows
	...
	function lookNumberDetail(number : string) {
		uni.$u.route('pages/openorder/Financial/Transfer/TransferDetail',
			{ number: number });
	}
	//新增
	function onBtnClick() {
		uni.$u.route('pages/openorder/Financial/Transfer/AddTransfer');
	}
	//编辑
	function goAdvanceInDetail(item) {
		uni.$u.route('/pages/openorder/Financial/Transfer/AddTransfer?item=' + encodeURIComponent(JSON.stringify(item)) + '&action=edit')
	}
	//分页切换
