	...
	<view style="min-height: 100vh;">
		...
		<view class="popup-title">
			<u-form :model="myform" label-width="160rpx" :rules="rules" ref="uFormRef" label-align="right"
									...
									<view class="good-item" style="min-height: 80rpx;">
										...
										<u-col span="12">
											<view class="goods-row">
												...
												<text class="value">{{ item.price || '0' }}</text>
											</view>
										</u-col>
										<u-col span="12">
											<view class="goods-row">
				...
				<!-- 订单表尾 -->
				<u-form-item label-align="right" label="付款账户：" prop="account">
					<u-input v-model="myform.account" placeholder="请选择付款账户" @click="accountShow = true"
						:type="type" :disabled="true" />
					<u-select v-model="accountShow" :list="accountList" @confirm="paymentConfirm"></u-select>
				</u-form-item>
...
<script setup lang="ts">
	...
	const getOrderNo = async () => {
		...
		if (res && res.code === 200) {
			billno.value = "ZZ" + res.data.defaultNumber;
		}
	}
	...
	const loadGetAccountList = async () => {
		...
		if (res && res.code === 200) {
			...
			accountDetail.value.accountId = accountId.value;
		}
		else {
	...
	function accountDetailSubmit() {
		...
		const newAccountData = {
			...
			price: accountDetail.value.price,
	...
	//新增取消
	function Cancel() {
		uni.$u.route('pages/openorder/Financial/Transfer/Transfer');
	}
	//新增保存
	const SavePurchaseIn = async () => {
		...
		const totalAmount = accountDetailList.value.reduce((sum, item) => {
			const eachAmount = Number(item.price) || 0;
