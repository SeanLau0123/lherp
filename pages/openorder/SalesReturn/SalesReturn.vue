...
<script setup lang="ts">
	...
	const click = (index : number, index1 : number) => {
		...
		if (index1 === 2) {
			...
			goSalesReturnDetail(curItem); // 调用已有编辑方法，传递当前单据数据
		} else {
	...
	const loadSalesReturnList = async () => {
		let params = {
	...
	//新增
	function onBtnClick() {
		uni.$u.route('pages/openorder/SalesReturn/AddSalesReturn');
	}
	//编辑
	function goSalesReturnDetail(item) {
		uni.$u.route('/pages/openorder/SalesReturn/AddSalesReturn?item=' + encodeURIComponent(JSON.stringify(item)) + '&action=edit')
	}
	//分页切换
