	...
	<view style="min-height: 100vh;">
		...
		<view class="popup-title">
			<u-form :model="myform" label-width="165rpx" :rules="rules" ref="uFormRef" label-align="right"
				...
				<u-form-item label-align="right" label="客户：" prop="supplier">
					<u-input v-model="myform.supplier" :type="type" placeholder="请选择客户"
...
<script setup lang="ts">
	...
	const getOrderNo = async () => {
		...
		if (res && res.code === 200) {
			billno.value = "XSTH" + res.data.defaultNumber;
		}
	}
	...
	const SavePurchaseIn = async () => {
		...
		if (isEdit.value === true) {
			...
			if (res.code === 200) {
				...
				uni.$u.route('pages/openorder/SalesReturn/SalesReturn');
			} else {
