...
<template>
	<view style="height: 100vh;">
		...
		<u-sticky>
			<view class="sticky">
				<u-text :bold="true" text="出库总数量："></u-text>
				<u-text :bold="true" :text="OutSummarySumTotal"></u-text>

				<u-text :bold="true" text="，出库总金额："></u-text>
				<u-text :bold="true" mode="price" :text="OutSummaryPriceTotal"></u-text>
			</view>
		</u-sticky>
		<scroll-view class="scrollviewpadding">
			...
			<view v-for="(OutSummary, index) in OutSummaryList" :key="OutSummary.barCode || index">
				<view class="good-item">
					<u-row gutter="10">
						<u-col span="12">
							<u-collapse :head-style="headStyle">
								<u-collapse-item :title="OutSummary.mName">
									<view class="goods-row">
										<text class="label">条码：</text>
										<u-text :text="OutSummary.barCode"></u-text>
									</view>
									<view class="goods-row">
										<text class="label">品牌：</text>
										<u-text :text="OutSummary.brand"></u-text>
										<text class="label">制造商：</text>
										<u-text :text="OutSummary.mfrs"></u-text>
									</view>
									<view class="goods-row">
										<text class="label">规格：</text>
										<u-text :text="OutSummary.standard"></u-text>
										<text class="label">型号：</text>
										<u-text :text="OutSummary.model"></u-text>
									</view>
						...
						<u-col span="12">
							<view class="goods-row">
								...
								<u-text :text="OutSummary.categoryName"></u-text>
								<text class="label">单位：</text>
								<u-text :text="OutSummary.materialUnit"></u-text>
							</view>
						</u-col>
						<u-col span="5">
							<view class="goods-row">
								<text class="label">出库数量：</text>
								<u-text :text="OutSummary.numSum"></u-text>
							</view>
						</u-col>
						<u-col span="7">
							<view class="goods-row">
								<text class="label">出库金额：</text>
								<u-text mode="price" :text="OutSummary.priceSum"></u-text>
							</view>
						</u-col>
...
<script setup lang="ts">
	...
	const depotId = ref("");
	const OutSummarySumTotal = ref(0);
	const OutSummaryPriceTotal = ref(0);
	...
	const OutSummaryList = ref<Array>([]);
	const searchname = ref<string>('');
	const loadOutSummaryList = async () => {
		let params = {
			materialParam: '',
		...
		if (res && res.code === 200) {
			...
			OutSummaryList.value = res.data.rows
			OutSummarySumTotal.value = res.data.numSumTotal
			OutSummaryPriceTotal.value = res.data.priceSumTotal
