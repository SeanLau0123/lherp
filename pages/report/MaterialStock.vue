...
<template>
	<view style="height: 100vh;">
		...
		<u-sticky>
			<view class="sticky">
				<u-text :bold="true" text="总库存："></u-text>
				<u-text :bold="true" :text="currentStock"></u-text>
				<u-text :bold="true" text="，总库存金额："></u-text>
				<u-text :bold="true" mode="price" :text="currentStockPrice"></u-text>
			</view>
		</u-sticky>
		<scroll-view class="scrollviewpadding">
			...
			<view style="min-height: 80rpx;" v-for="(stock, index) in materialStockList" :key="stock.barCode || index">
				<view class="good-item">
					<u-row gutter="10">
						<u-col span="12">
							<u-collapse :head-style="headStyle">
								<u-collapse-item :title="stock.name">
									<view class="goods-row">
										<text class="label">条码：</text>
										<u-text :text="stock.mBarCode"></u-text>
									</view>
									<view class="goods-row">
										<text class="label">品牌：</text>
										<u-text :text="stock.brand"></u-text>
										<text class="label">制造商：</text>
										<u-text :text="stock.mfrs"></u-text>
									</view>
									<view class="goods-row">
										<text class="label">规格：</text>
										<u-text :text="stock.standard"></u-text>
										<text class="label">型号：</text>
										<u-text :text="stock.model"></u-text>
									</view>
						...
						<u-col span="12">
							<view class="goods-row">
								...
								<u-text :text="stock.categoryName"></u-text>
							</view>
						</u-col>
						<u-col span="12">
							<view class="goods-row">
								<text class="label">单位：</text>
								<u-text :text="stock.unitName"></u-text>
								<text class="label">成本价：</text>
								<u-text mode="price" :text="stock.purchaseDecimal"></u-text>
							</view>
						</u-col>
						<u-col span="12">
							<view class="goods-row">
								<text class="label">初始库存：</text>
								<u-text :text="stock.initialStock"></u-text>
								<text class="label">库存：</text>
								<u-text :text="stock.currentStock"></u-text>
							</view>
						</u-col>
						<u-col span="12">
							<view class="goods-row">
								<text class="label">库存金额：</text>
								<u-text mode="price" :text="stock.currentStockPrice"></u-text>
							</view>
						</u-col>
						<u-col span="12">
							<view class="goods-row">
								<text class="label">库存详情：</text>
								<u-text type="primary" decoration="underline" text="流水"
									@click="lookRecord(stock.id)"></u-text>
								<u-text type="primary" decoration="underline" text="分布"
									@click="lookDetail(stock.mBarCode,stock.name,stock.id,stock.purchaseDecimal)"></u-text>
							</view>
						</u-col>
					</u-row>
				</view>
...
<script setup lang="ts">
	...
	const loadgetMaterialStock = async () => {
		...
		if (res && res.code === 200) {
			...
			materialStockList.value = res.data.rows
			currentStock.value = res.data.currentStock
			currentStockPrice.value = res.data.currentStockPrice
