...
<template>
	<view style="height: 100vh;">
		...
		<u-sticky>
			<view class="sticky">
				<u-text :bold="true" text="入库总数量："></u-text>
				<u-text :bold="true" :text="inSummarySumTotal"></u-text>

				<u-text :bold="true" text="，入库总金额："></u-text>
				<u-text :bold="true" mode="price" :text="inSummaryPriceTotal"></u-text>
			</view>
		</u-sticky>
		<scroll-view class="scrollviewpadding">
			...
			<view v-for="(inSummary, index) in inSummaryList" :key="inSummary.barCode || index">
				<view class="good-item">
					<u-row gutter="10">
						<u-col span="12">
							<u-collapse :head-style="headStyle">
								<u-collapse-item :title="inSummary.mName">
									<view class="goods-row">
										<text class="label">条码：</text>
										<u-text :text="inSummary.barCode"></u-text>
									</view>
									<view class="goods-row">
										<text class="label">品牌：</text>
										<u-text :text="inSummary.brand"></u-text>
										<text class="label">制造商：</text>
										<u-text :text="inSummary.mfrs"></u-text>
									</view>
									<view class="goods-row">
										<text class="label">规格：</text>
										<u-text :text="inSummary.standard"></u-text>
										<text class="label">型号：</text>
										<u-text :text="inSummary.model"></u-text>
									</view>
						...
						<u-col span="12">
							<view class="goods-row">
								...
								<u-text :text="inSummary.categoryName"></u-text>
								<text class="label">单位：</text>
								<u-text :text="inSummary.materialUnit"></u-text>
							</view>
						</u-col>
						<u-col span="5">
							<view class="goods-row">
								<text class="label">入库数量：</text>
								<u-text :text="inSummary.numSum"></u-text>
							</view>
						</u-col>
						<u-col span="7">
							<view class="goods-row">
								<text class="label">入库金额：</text>
								<u-text mode="price" :text="inSummary.priceSum"></u-text>
							</view>
						</u-col>
...
<script setup lang="ts">
	...
	import { getInOutSummary } from '@/api/api.js'
	import { $u, useTheme } from 'uview-pro'
	const { currentTheme, themes, darkMode } = useTheme();
	const title = ref<string>('入库汇总')
	...
	const depotId = ref("");
	const inSummarySumTotal = ref(0);
	const inSummaryPriceTotal = ref(0);
	...
	const inSummaryList = ref<Array>([]);
	const searchname = ref<string>('');
	const loadInSummaryList = async () => {
		let params = {
			materialParam: '',
		...
		const res = await getInOutSummary(params)
		if (res && res.code === 200) {
			listTotal.value = res.data.total
			inSummaryList.value = res.data.rows
			inSummarySumTotal.value = res.data.numSumTotal
			inSummaryPriceTotal.value = res.data.priceSumTotal
